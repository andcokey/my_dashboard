...
- 「DashboardMailUpdate」→ `%USERPROFILE%\.claude\scripts\update-dashboard-mail.ps1`
  - Microsoft 365 MCPで koki-itabashi@group.gmo の受信メールを取得し、自動送信・メルマガを除外した上でSent Itemsとの照合で未返信を判定
    （nishiyama-sec@gmo.jpは同メールボックス宛のエイリアスのため、Inbox検索一回で自動的に含まれる。委任アクセスは不要）
...
西山さん秘書用アドレス nishiyama-sec@gmo.jp は別メールボックスではなく koki-itabashi@group.gmo 宛の
エイリアス（確認済み）。委任アクセスは不要で、既存のInbox検索一回でこのアドレス宛メールも監視対象に含まれている。
